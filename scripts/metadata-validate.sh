...
for field in title abstract motivation rationale; do
    ...
    if [ -n "$text" ]; then
        ...
        # Filter out governance action IDs (gov_action1...) and other Cardano identifiers
        echo "$text" | aspell list $PERSONAL_DICT_ARG | grep -v '^gov_action1[a-z0-9]*$' | sort -u | while read -r word; do
            if [ -n "$word" ]; then
